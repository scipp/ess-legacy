...
def load_calibration(filename, mantid_args={}):
    ...
    with scn.mantid.run_mantid_alg('LoadDiffCal', Filename=filename, **mantid_args) as output:
...
def validate_dataset(dataset):
    ...
    check_units = re.split('[*/]', dataset.unit)
...
# ----------------
# WORK IN PROGRESS: needs to deal with bins
# ----------------
def convert_with_calibration(dataset, cal):
    ...
    if "detector_info" in list(dataset.coords.keys()):
        ...
        detector_info = dataset.coords["detector_info"].value
    ...
    if 'tof' in list(dataset.coords.keys()):
        output.coords['tof'] = (output.coords['tof'] - cal["tzero"].data) / cal["difc"].data
